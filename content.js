const CODENAME = 'b1b'
const VARPAGESCROLL = 'perc'
const TIMEOUT_POPUP = 350 // time for the pop-up to appear
const TIMEOUT_SNACKBAR = 250 // time for the snack bar (feedback) to be added to the page
const DURATION_SNACKBAR = 3000

// svg code for the icon on the scholar page
...
let showSnackbar = () => {
    ...
    setTimeout(() => { divSnackbar.className = divSnackbar.className.replace("show", ""); }, DURATION_SNACKBAR);
}
...
// if this is a bibtext page, copy the bibtex string and go back
if (url.includes('scholar.googleusercontent') && getParamFromUrl(CODENAME) == 1) {
...
// if this is a scholar search results page
    ...
    if (getParamFromUrl(CODENAME) == 1) {
        // automatically scrolling to the previous page height
        if ('scrollRestoration' in history) {
            history.scrollRestoration = 'manual'
            window.scrollTo(0, getParamFromUrl(VARPAGESCROLL))
        }
        // show snackbar
        setTimeout(() => { showSnackbar() }, TIMEOUT_SNACKBAR);
        // clear the flag
        history.replaceState({ info: CODENAME + ' state' }, document.title, setParamInUrl(CODENAME, 0))
    }
        ...
        if (divElm.className == 'gs_fl') {
            ...
            aBib.addEventListener('click', () => {
                ...
                Array.from(siblings).forEach(elm => {
                    if (elm.className == 'gs_or_cit gs_nph') { // the citation button (double-quotation icon)
                        elm.click()
                        setTimeout(() => {
                            aCitations = document.getElementsByClassName('gs_citi')
                            Array.from(aCitations).forEach(elm => {
                                // locate the bibtex link
                                if (elm.href.includes('scholar.bib')) { // the bibtex link
                                    // add the bib and scroll perc param to url for passing info between pages
                                    const queryString = location.search
                                    const urlParams = new URLSearchParams(queryString)
                                    urlParams.set(CODENAME, 1)
                                    urlParams.set(VARPAGESCROLL, dy)
                                    history.replaceState({ info: CODENAME + ' state', scrollTop: dy }, document.title, location.pathname + '?' + urlParams)
                                    location.href = elm.href + '&' + CODENAME + '=1'
                                }
                            })
                        }, TIMEOUT_POPUP);
                        return
